...
- 朗读页在隐藏状态栏时仍会避开刘海与前置摄像头安全区；系统悬浮球移除异常外投影，并新增三态交互：闲置后缩成准确的贴边半圆，点击展开封面、暂停继续和停止控制，长按封面展开接近屏幕宽度的原文窗口。
- 悬浮原文窗口支持当前句持续居中、顶部章节切换、返回完整朗读页、缩小回贴边球，并统一使用应用主题圆角；可独立调整原文字号、35%–90% 窗口高度和 0%–100% 背景透明度。
- 修复完整朗读面板在前台时隐藏悬浮窗的状态没有随朗读页退到后台释放、导致离开应用后悬浮窗不出现的问题；返回朗读页后仍会自动隐藏，主题重建期间不会误闪。
- 本次未修改数据库结构，无需数据库迁移。
